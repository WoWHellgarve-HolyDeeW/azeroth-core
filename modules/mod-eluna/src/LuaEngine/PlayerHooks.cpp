...
void Eluna::OnChangeSpec(Player* pPlayer, uint8 specId)
{
    START_HOOK(PLAYER_EVENT_ON_CHANGE_SPEC);
    Push(pPlayer);
    Push(specId);
    CallAllFunctions(PlayerEventBindings, key);
}
...
void Eluna::OnLevelUp(Player* pPlayer)
{
    START_HOOK(PLAYER_EVENT_ON_LEVEL_UP);
    Push(pPlayer);
    CallAllFunctions(PlayerEventBindings, key);
}
