...
namespace LuaPlayer
{
    ...
    int SetGamemode(lua_State* L, Player* player)
    {
        uint32 modeId = Eluna::CHECKVAL<uint32>(L, 2);
        Gamemode::SetGameMode(player, modeId);
        return 0;
    }

    int RemoveGamemode(lua_State* L, Player* player)
    {
    ...
    int HasGamemode(lua_State* L, Player* player)
    {
        uint32 mode = Gamemode::GetGameMode(player->GetGUID());
        Eluna::Push(L, mode != 0);
        return 1;
    }
    ...
    int InDungeon(lua_State* L, Player* player)
    {
        Eluna::Push(L, player->GetMap()->IsDungeon() || player->GetMap()->IsRaid());
        return 1;
    }
#endif
    ...
    int InSanctuary(lua_State* L, Player* player)
    {
        Eluna::Push(L, player->IsInSanctuary());
        return 1;
    }
    ...
    int GetPrestigeLevel(lua_State* L, Player* player)
    {
        Eluna::Push(L, PerksManager::GetPrestigeLevel(player));
        return 1;
    }


    int GetLastestPerksForLevelUp(lua_State* L, Player* player)
    {
        lua_newtable(L);
        int tbl = lua_gettop(L);
        uint32 i = 0;

        std::vector<PerksManager::CharacterPerk> lastestPerk = PerksManager::GetLastestPerk(player, 0);

        if (lastestPerk.size() == 0)
            return 1;

        Eluna::Push(L, lastestPerk[0].uuid);
        lua_rawseti(L, tbl, ++i);

        for (auto playerPerk : lastestPerk) {
            if (!playerPerk.isDefault) {
                PerksManager::CharacterPerk* perk = &playerPerk;
                Eluna::Push(L, perk->spellId);
                lua_rawseti(L, tbl, ++i);
            }
        }
        lua_settop(L, tbl);
    ...
    {
        PerksManager::ResetCharacter(player);
        return 0;
    }

    int GetPreviousPerks(lua_State* L, Player* player)
    {
        lua_newtable(L);
        int tbl = lua_gettop(L);
        uint32 i = 0;

        std::vector<PerksManager::PerkPoll> lastestPerk = PerksManager::GetLastPlayersPerks(player);

        if (lastestPerk.size() == 0)
            return 1;

        for (auto playerPerk : lastestPerk) {
            Eluna::Push(L, playerPerk.spellId);
            lua_rawseti(L, tbl, ++i);
        }
        lua_settop(L, tbl);
        return 1;
    ...
    int GetPlayerPerks(lua_State* L, Player* player)
    {
        lua_newtable(L);
        int tbl = lua_gettop(L);
        uint32 i = 0;
        ...
        for (auto perk : PerksManager::CharacterPerks[player->GetGUID().GetCounter()])
            if (perk.isChosen && perk.specId == player->GetActiveSpec()) {
                Eluna::Push(L, perk.spellId);
                lua_rawseti(L, tbl, ++i);
            }
        lua_settop(L, tbl);
    ...
    int SetSelectPerk(lua_State* L, Player* player)
    {
        std::string UUID = Eluna::CHECKVAL<std::string>(L, 2);
        uint32 spellId = Eluna::CHECKVAL<uint32>(L, 3);
        PerksManager::SetSelectPerk(player, UUID, spellId);
        return 0;
    }
    /**
    ...
    int SetWarmode(lua_State* L, Player* player)
    {
        bool apply = Eluna::CHECKVAL<bool>(L, 2, true);
        if (apply)
        {
            if (player->getLevel() < 80) {
                player->RemoveAura(1000003);
                player->AddAura(1000002, player);
            }
            else {
                player->RemoveAura(1000002);
                player->AddAura(1000003, player);
            }
    ...
    int ResetLockout(lua_State* L, Player* player)
    {
        if (player->GetMap()->IsRaid() || player->GetMap()->IsDungeon()) {
            player->GetSession()->SendAreaTriggerMessage("You can't do that while in dungeon or raid.");
            return 0;
        }
        uint16 counter = 0;
        ...
        player->GetSession()->SendAreaTriggerMessage("Your raid and instance lockout as been reset.");
        return 0;
    }



    int RemoveFFA(lua_State* L, Player* player)
    {
        player->RemoveByteFlag(UNIT_FIELD_BYTES_2, 1, UNIT_BYTE2_FLAG_FFA_PVP);
        return 0;
    }
    ...
    int GetItemEnchantSpellId(lua_State* L, Player* player)
    {
        uint32 bag = Eluna::CHECKVAL<uint32>(L, 2);
        uint32 slot = Eluna::CHECKVAL<uint32>(L, 3);
        Eluna::Push(L, RandomEnchant::GetItemEnchantSpellId(player, bag, slot));
        return 1;
    }

    int ApplyRandomEnchant(lua_State* L, Player* player)
    {
        uint32 bag = Eluna::CHECKVAL<uint32>(L, 2);
        uint32 slot = Eluna::CHECKVAL<uint32>(L, 3);
        uint32 spellId = Eluna::CHECKVAL<uint32>(L, 4);
        Item* pItem = RandomEnchant::GetItemByPosition(player, bag, slot);
        uint32 enchantId = RandomEnchant::SpellIdToEnchantId(spellId);
        RandomEnchant::ApplyRandomEnchant(pItem, enchantId, player);
        return 1;
    }

    int GetPossiblesEnchants(lua_State* L, Player* player)
    {
        uint32 bag = Eluna::CHECKVAL<uint32>(L, 2);
        uint32 slot = Eluna::CHECKVAL<uint32>(L, 3);
        std::vector<uint32> spellIds = RandomEnchant::GetPossibleEnchantsToApply(player, bag, slot);
