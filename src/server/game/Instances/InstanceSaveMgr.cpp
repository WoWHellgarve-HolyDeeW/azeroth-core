...
void InstanceSaveMgr::PlayerUpdateBoundInstanceMythic(ObjectGuid guid, Difficulty difficulty, uint32 mapId, bool mythic)
{
    auto itr = playerBindStorage.find(guid);
    if (itr != playerBindStorage.end()) {
        itr->second->m[difficulty][mapId].mythic = mythic;
        uint32 instanceId = itr->second->m[difficulty][mapId].save->GetInstanceId();
        CharacterDatabase.Query("UPDATE character_instance SET mythic = %u WHERE instance = %u AND guid = %u", mythic, instanceId, guid.GetCounter());
    }
}
